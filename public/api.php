...
function validate_contracts($data, $dataDir, &$masters)
{
    ...
    foreach ($data['contracts'] as $c) {
        ...
        if (!in_array($status, ['active', 'pending', 'ended'], true)) {
            $errors[] = "{$id}: status は active / pending / ended のいずれかにしてください（現在: {$status}）";
        }
    ...
    $live = [];
    foreach ($data['contracts'] as $c) {
        if ((isset($c['status']) ? $c['status'] : '') !== 'ended' && isset($c['towns']) && is_array($c['towns'])) {
            $live[] = $c;
        }
    }
...
switch ($action) {
    ...
    case 'save':
        ...
        atomic_write($STORE_FILE, $storeJson);
        // バックアップ（直近 BACKUP_KEEP 世代）
        file_put_contents($PRIVATE_DIR . '/backup/store-' . date('Ymd-His') . '.json', $storeJson);
        $backups = glob($PRIVATE_DIR . '/backup/store-*.json');
        sort($backups);
        ...
        $summary = generate_summary($data, $DATA_DIR, $MASTERS);
        atomic_write($DATA_DIR . '/taken.json', json_encode($taken, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT) . "\n");
        atomic_write($DATA_DIR . '/summary.json', json_encode($summary, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT) . "\n");

        flock($lock, LOCK_UN);
        respond(200, [
            'ok' => true,
            'revision' => $newRevision,
            'taken' => count($taken['hashes']),
            'municipalities' => count($summary['municipalities']),
        ]);
        // no break
